...
- `dock.visible` defaults to `false`. Set it to `true` to show the right-hand dock at startup, or run **Toggle Right Dock** from the command palette (the runtime toggle does not rewrite config). The dock exposes **Session**, **Agents**, and **Review**. Agents is a live, observational board with one card per detected agent pane; each card shows provider identity, tab/workspace context, current activity, and a working/waiting/done/error state, and opens the exact originating pane when selected. Review shows the selected local workspace's exact machine/repository/worktree identity, base and HEAD revisions, bounded committed/staged/unstaged/untracked diffs, revision-aware viewed markers, authoritative task progress, and verified PR/check state. Both views are observational and do not replace external task or PR truth.
