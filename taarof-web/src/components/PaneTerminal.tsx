...
import { readTerminalBufferText } from "../terminalBufferText";
...
export interface PaneTerminalProps {
  ...
  /** Only the focused tile can own the mutating control stream. */
  isSelected: boolean;
}
...
export function PaneTerminal({
  ...
  const lastResetIdentityRef = useRef<{ key: string; host: unknown } | null>(null);
  ...
  const tileWorkRef = useRef(new PaneTileWork());
  ...
  const legacyTransport = planPaneTerminalTransport({
    hasTerminalHost: Boolean(terminalHost),
    hasTab: Boolean(tab),
    hasPane: Boolean(activePane),
    attachSupported,
    usePtyTransport,
    isSelected,
    isControlUnlocked,
  ...
  // Legacy observation remains open for every mounted tile. Unlocking the
  // selected tile adds a control socket; it never replaces the observe socket.
  const controlSocketEnabled = legacyTransport.controlEnabled;
  const attachEnabled = legacyTransport.observeEnabled;
  const canUseControl =
    isSelected && isControlUnlocked && hasControlTarget && attachSupported;
  ...
  // A tile owns all of its delayed work. Changing pane identity or token (and
  // unmounting on overflow, tab/view change, or auth reset) aborts requests and
  // cancels callbacks before they can update the next tile or revoke its auth.
  useEffect(() => {
    setClipNote(null);
    setFilePreview({ status: "idle" });
    return () => {
      tileWorkRef.current.cancelTimer(resizeDebounceRef.current);
      resizeDebounceRef.current = undefined;
      tileWorkRef.current.cancel();
    };
  }, [paneId, tabId, token]);
  ...
  pathReferenceActivationRef.current = (reference: PathReferenceActivation) => {
    ...
    const controller = tileWorkRef.current.beginRequest("file-preview");
    void fetchFilePreview(
      context.token,
      {
        tabId: context.tabId,
      ...
      .finally(() => {
        tileWorkRef.current.completeRequest("file-preview", controller);
      });
  };
  ...
  const dismissTerminalLinkChoices = () => {
    ...
    if (decision.restoreFocus) {
      tileWorkRef.current.schedule(() => terminalRef.current?.focus(), 0);
    }
  };
  ...
  terminalLinkActivationRef.current = (
    ...
    const context = filePreviewContextRef.current;
    const controller = tileWorkRef.current.beginRequest("file-stat");
    void resolveTerminalLinkForPane(rawLink, async (path) => {
      if (!context) {
        return "indeterminate";
      }
      try {
        const stat = await fetchFileStat(
          context.token,
          {
            tabId: context.tabId,
            paneId: context.paneId,
      ...
      tileWorkRef.current.completeRequest("file-stat", controller);
      if (controller.signal.aborted) {
        return;
      }
  ...
  const scheduleResizeDispatch = useCallback(() => {
    tileWorkRef.current.cancelTimer(resizeDebounceRef.current);
    resizeDebounceRef.current = tileWorkRef.current.schedule(() => {
      resizeDebounceRef.current = undefined;
      const terminal = terminalRef.current;
      if (!terminal) {
        return;
  ...
  useEffect(() => {
    ...
    // The observe socket is the sole legacy renderer. The control endpoint
    // mirrors snapshots/deltas for standalone clients, but rendering either
    // stream here would duplicate output and its initial snapshot would reset
    // scrollback every time control is unlocked.
    const observeControlFrame = () => {
      if (active) {
        applyLiveFrame();
      }
    };
  ...
  useEffect(() => {
    if (!isSelected && isControlUnlocked) {
      setIsControlUnlocked(false);
      setControlSocketState(attachSupported ? "observe" : "unsupported");
      setControlMessage("Observe-only mode is active for this unselected pane.");
    }
  }, [attachSupported, isControlUnlocked, isSelected]);
  ...
  return (
    <section
      aria-labelledby={`pane-stage-heading-${tabId ?? "none"}-${paneId ?? "none"}`}
      className={
        isSelected
          ? "pane-view pane-stage__tile pane-stage__tile--selected"
          : "pane-view pane-stage__tile"
      }
      onFocusCapture={() => {
        if (paneId !== null) onSelectPane(paneId);
      }}
      onPointerDown={() => {
        if (paneId !== null) onSelectPane(paneId);
      }}
    ...
    >
      ...
      <div className="pane-view__header">
        <div>
          <h2 id={`pane-stage-heading-${tabId ?? "none"}-${paneId ?? "none"}`}>
            {tab.name} · Pane {paneId ?? "?"}{isSelected ? " (selected)" : ""}
          </h2>
      ...
      <div className="pane-view__mobile-note">
        Phone view stacks every visible pane, starts in observe mode, and still
        requires the local bearer token after browser storage resets.
      ...
      >
        ...
        <div className="pane-view__control-actions">
          <button
            disabled={!isSelected || !hasControlTarget || !attachSupported}
      ...
      <div className="terminal-surface">
        <div className="terminal-surface__header">
          <div>
            ...
            <p aria-live="polite">{attachMessage}</p>
          </div>
          <div className="terminal-surface__badges">
            ...
            <span
              ...
              role={attachPhase === "error" ? "alert" : undefined}
            >
